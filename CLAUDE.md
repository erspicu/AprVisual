...
- **S4 — codegen (+ GPU)**: **not started.** IR → an executable bit-sliced model (the ~85% as a flat program; residual SCCs + hybrid buses as fixed-K micro-blocks). Two CPU backends built in parallel for a perf comparison: a C# one (Roslyn-emit C# source) and an LLVM one (LLVMSharp / LLVM-C P/Invoke → LLVM IR → `-O3`); the codegen tool itself stays C#. GPU target TBD — CUDA/NVPTX isn't the only option (GPU compute shaders are a candidate; the user's `AprNesAvalonia` is a reference). Then benchmark vs S1. Design draft: `MD/impl/S4/00_codegen_設計.md`. (The roadmap `MD/struct/08` was revised in 2026-05 — see "路線圖調整 #2" and "目標與最終形態".)
- **After S4 — a `cpu-opt` branch** (forked from an earlier commit, ~end of S3): an *event-driven* IR interpreter (dirty-set, re-eval only the nodes whose inputs changed — the "β" approach Gemini deprioritized for GPU); the fastest single-instance-on-CPU version. Built after the GPU version succeeds.
