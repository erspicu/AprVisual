...
| 2026-07-18 | **False-alarm retraction: isolated arms used the wrong recipe (K=0); 146/1 stands** | During the S1a decidability sweep, plain isolated reruns (**missing `--reset-hold-extra 1`**) made oam_read/oam_stress/05-nmi FAIL, briefly misread as "legal lottery re-roll, expected 145/2" → **fully retracted**. Settled: the campaign runner passes K=1 to **every** test (run_tests.py L154), so K=0 arms cannot be compared across eras (05-nmi was already documented to fail at K∈{0,5}). Supporting chain: S1 ≡ S1A four arms hc-identical (zero fork drift — this part still holds); the t13032b-removed forensic arm was hc-identical too (and the ALERead node-split is opt-in, never cut in plain runs → both "graph-change re-roll" attributions disproven in turn). **146/1 remains valid**; full reruns on the latest S1 build (AC 141 + 147) are in flight to re-certify. **Lesson: before any isolated re-verification, check the runner source for the recipe it actually used** |
