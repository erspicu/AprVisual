...
- **S4.6 — GPU compute shader**: **explored; the codegen + the equivalence-verified GPU step are done; the fast (all-on-GPU) version is parked.** D3D11 compute via Vortice (`Vortice.Direct3D11` + `Vortice.D3DCompiler` NuGet, headless, no window — `--gpu-spike` verified on the RTX 4080). The IR's EvalOrder is topo-layered (`IrEngine.BuildTopoLevels` — 44 levels, ~265-wide avg; `--dump-topo-levels`) and codegen'd into an HLSL bytecode-interpreter kernel (thread = one node, runs that node's RPN bytecode, `AllMemoryBarrierWithGroupSync` between levels; `GpuCodegen.EmitHlslCompute` / `--dump-emitted-hlsl`). The GPU step (the DAG eval, a drop-in for `StepOneDriving`'s step-4 with S1 still doing step-5) is equivalence-verified (`--trace-cmp --engine ir --gpu-step` 0 mismatch). **Benchmark**: ~264 s/frame ≈ 2475 hc/s — ~18× *slower* than S1, because the per-step CPU↔GPU round-trip (upload the whole node-state + dispatch + `Map(Read)` sync, every half-cycle) dominates. The fast version (state stays on GPU, N half-cycles per dispatch, memory handlers + bus resolver + SCC fixed-K all in HLSL) is parked — it hits the same within-half-cycle-event-sequencing obstacle that parked the ping-pong run-path (the PPU precharged-dynamic readout), so it wouldn't be equivalence-clean without a "2-phase precharge model" too. SKSL/AprNesAvalonia is *not* suitable for the compute (fragment-only). Plan + the firing log: `MD/impl/S4/01_S4.6_GPU_compute_設計.md`.
