...
### 6d. 頂點級定位:漏在「多邊形→器件萃取」,不在描圖(使用者追問 qmtpro 下載何處有問題)

segdefs 在空格座標的幾何**完整存在**:

```
空格(y=7338-7348):
  #10975 sw-diff 指  (6840,7338)-(6856,7348)   ← 右緣貼 poly 左緣
  _ab1   poly 直行    x∈[6856,6862], y 6492-7570
  vss    gnd-diff     x∈[6862,6884], y 6570-7520
對照(t12933 存在,y=7494-7520):
  #13214 sw-diff 指  頂到 x=6856(同構)→ transdefs 有列
```

「源極指→poly 閘→汲極入 vss」三件俱全,與隔壁真實器件逐頂點同構 ——
**die 照片 ✅、segdefs 描摹 ✅、transdefs 器件萃取 ❌(漏發一列)**。
含義:未來的 netlist-patch 不是無中生有,而是把 segdefs 已畫出的器件還原到
transdefs;佐證三重(segdefs 幾何 + BreakNES + 硬體行為)。
公平註記:10,946 列漏 1(>99.99% 正確);Visual6502 家族歷年有社群修正先例。
