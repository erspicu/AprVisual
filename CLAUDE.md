...
- **S4 — codegen + GPU**: **not started.** IR → an executable bit-sliced model (the ~85% as a flat program; residual SCCs + hybrid buses as fixed-K micro-blocks); backend preferred = LLVM-via-.NET (one IR retargets x86/ARM and NVPTX/CUDA — the codegen tool itself stays C#); then benchmark vs S1, then GPU. Design draft: `MD/impl/S4/00_codegen_設計.md`. (The roadmap `MD/struct/08` was revised in 2026-05 to reflect the S3 widening — see its "路線圖調整 #2".)
