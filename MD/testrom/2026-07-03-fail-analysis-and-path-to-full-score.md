...
>
> **2026-07-03 進度更新:A 類(上電)兩個已全數修復(117 PASS / 24 FAIL)** —— 見 §3 路線圖 #2 的「已完成」注記。
> **2026-07-04 進度更新:B 類(相位)結案(124 PASS / 17 FAIL)** —— 見 §3 路線圖 #1 的「已完成」注記。
> **2026-07-04 進度更新 2:open-bus 衰減 shim 完成(125 PASS / 16 FAIL)—— 路線圖 #1-#3「便宜且確定」區全部收官;剩餘 16 個 = trace 深究區 + 忠實偏差。**
...
|---|---|---|---|---|---|
| 1 | **時脈相位對齊(✅ 已完成 2026-07-04,淨 +7 兌現)**:`--phase-probe` 實測發現 **CPU ÷12 自由運轉、PPU ÷4 在 /res 釋放時重啟** → `--reset-hold-extra K` 可掃過全部 4 種真機對齊(相對相位 {1,7,5,3} ↔ K={0,1,3,5})。實測矩陣:NMI 邊沿家族 8 個在對齊 {7,5} 過、{1,3} 掛;`10-even_odd_timing` 恰好相反(在對齊 {1,3} 過)—— **2+2 完全互補、零交集**(四格矩陣 2026-07-04 補完)—— **四種對齊無一全滿足,與 NESdev 記載的真機行為一致**(even_odd 系列本來就看上電對齊,實機要重開機碰運氣)。定案:runner 預設 `--reset-hold-extra 1`(對齊 7),收復四連星 4 + 舊版三連 3 + nmi_and_brk 1;`10-even_odd_timing` 改列**對齊相依的忠實偏差**(有完整四對齊矩陣佐證);sprdma×2 排除相位、確定歸 E 類。引擎預設仍 0,benchmark/golden checksum 不動 | 淨 +7 ✅ | 中 | 低 —— 已文件化 | **完成** |
| 2 | **上電 shim(✅ 已完成 2026-07-03,+2 兌現)**:`WireCore.ApplyPowerUpState`(測試模式限定;benchmark/golden checksum 不受影響)。兩個實作發現值得記錄:(a) **調色盤 cell 是交叉耦合閂鎖**,單側驅動釋放後會被互補側扳回 —— 必須 `_a`/`_b` 兩側互補驅動→settle→雙側 SetFloat 釋放,cell 才保持且之後照常可寫(palette_ram 回歸 PASS 佐證);(b) **Z=1 不是上電殘值,而是 res 保持期間 flag 邏輯重新產生的** —— 上電後注入無效,必須在 /res 釋放後注入才保持(cpu.p1 單側即可) | +2 ✅ | 小 | 中 —— 已文件化 | **完成** |
| 3 | **PPU open-bus 衰減 shim(✅ 已完成 2026-07-04,+1 兌現)**:測試迴圈監測 `_io_db`(閂鎖側;`io_db` 是活的內部匯流排,值持續波動不可用 —— 診斷教訓),值穩定非零達 36 幀(~600ms)即驅動歸零+釋放。建模方式與 AccuracyCoin 作者的 TriCNES 相同(per-bit 計時器、其主機實測常數 1,786,830 cycles ≈1 秒)。**125/16 —— 便宜且確定區全部收官** | +1 ✅ | 小~中 | 中 —— 已文件化 | **完成** |
...
## 4. 忠實偏差的研究筆記與佐證連結(2026-07-04)

報告頁的「Faithful deviations」章節與本節同步;每一條都有社群/作者第一手佐證:
...
1. **OAM = DRAM(oam_read、cpu_dummy_writes_oam)**
   ...
   - **最強證據 —— blargg 自己的 oam_read readme**:他的 NTSC 前置式 NES 上電/重設後隨機出現四種圖樣,
     **其中三種是 Failed**(CRC 694ADBE0 / E9E8E60F / 44551956)。我們的 FAIL(CRC E03E03AD)屬於同一行為家族。
   - cpu_dummy_writes_oam 的畫面文字自述:前置需求(OAM 讀取可靠)在模擬器成立、「but NOT on the real NES」。

2. **CPU÷12/PPU÷4 上電對齊(10-even_odd_timing)**
   - [NESdev 論壇: CPU-PPU clock alignment](https://forums.nesdev.org/viewtopic.php?t=6186):「the beginning of a CPU tick could be offset by 0-3 master clock ticks」= 4 種對齊
   - [NESdev wiki: PPU frame timing](https://www.nesdev.org/wiki/PPU_frame_timing):奇數幀 dot-skip 行為
   - 我們的貢獻:`--phase-probe` 實測 **CPU 除頻器自由運轉、PPU 除頻器 /res 釋放時重啟** →
     `--reset-hold-extra` 可確定性列舉全部 4 種對齊;實測矩陣顯示 NMI 邊沿家族(對齊 {7,5} 過)與
     even_odd_timing(對齊 {1,3} 過)**完全互補、無交集** —— 真機也是分次上電各自碰對齊才全過。

3. **PPU open-bus 衰減(ppu_open_bus,待修)**
   - blargg readme 原文:「If a bit isn't refreshed with a 1 for about 600 milliseconds, it will decay to 0
     (**some decay sooner, depending on the NES and temperature**)」—— 溫度相依的類比洩漏;
     開關級 float=hold 模型無洩漏項,計畫用文件化行為計時器補。

4. **上電狀態(已修,shim)**
   - [NESdev wiki: PPU power up state](https://www.nesdev.org/wiki/PPU_power_up_state):palette 上電「unspecified」;
     power_up_palette 檢查的是 blargg 那台的殘值(其表成為模擬器共識)。
