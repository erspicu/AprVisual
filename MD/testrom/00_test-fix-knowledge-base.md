...
| 2026-07-18 | **oam_read legally re-rolled (current build expected 145/2)** | Found mid-sweep during the S1a decidability work: isolated `oam_read` now FAIL(1) with pattern E03E03AD (it PASSed in the campaign). Provenance settled with four arms: **the shipping S1 exe shows the same pattern at the same hc=20,667,239; S1 ≡ S1A bit-exact** (S1A default / ×NO_M2DECAY / ×no-oamdma all hc-identical) → the re-roll comes from a load-time graph change common to both forks (the ALERead node-split in LoadSystem; the Socket-Pattern lottery re-roll), **not a regression in either fork**. The §"oam_read" prophecy held (the lottery is the faithful behavior; it may legally change sides). **146/1 remains the campaign build's historical score**; the current build's expected 147-set score is 145/2, to be recorded at the next full regression |
