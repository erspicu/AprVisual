...
├── run_accuracycoin.bat    【另一電池】AccuracyCoin 141 無人版一鍵(不同判定路徑:$07F0 完成區塊,非 run_tests.py 流程;含 ALEREAD_MUX 等 AC 專屬設定)
├── ac_snap_results.py      AC 專用:從快照直讀 $04xx 結果表(飛行中看成績,對 oracle 逐位比對)
├── ac_watch.py             AC 專用:每 N 幀寄進度信(附截圖);判決權威 = AccuracyCoin.json
...
> ⏱️ **這會跑好幾個小時**(開關級模擬很慢,~5 秒/模擬幀;完整一輪在 7 綁核約 **6.2 小時**,見 §0)。中途 Ctrl+C 可中止。
...
- **另一電池:AccuracyCoin 141**(更狠、針對 RP2A03G+RP2C02G):一鍵 `run_accuracycoin.bat`、
  執行配方 `MD/memory/00_baseline-and-run-recipes.md`、報告 [ReportAC](https://erspicu.github.io/AprVisual/ReportAC/)。
  判定路徑不同(CPU RAM `$07F0` 完成區塊),且需 AC 專屬 env `ALEREAD_MUX`(漏了掉 141→140)。
