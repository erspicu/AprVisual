...
- `proto/` —— 原型實作與量測
  - `01_s2a_inline_adjacency.md` —— **S2-A 結果:bit-exact + 贏 S1 +4.18%(7/7 配對全勝)**

## 目前進度(2026-05-31)

**S2-A(節點鄰接內聯)已初步打敗 S1**:整機 full_palette 300k,bit-exact(checksum `0x794A43ABDF169ADA`
不變),interleaved-paired 8 輪 **7/7 全勝**,中位 **+4.18%**(81.7K vs 77.8K hc/s)。詳見 `proto/01`。

> ⚠ **釐清:S2-A 不是 IR。** 它是把**同一個 switch-level 事件驅動引擎**的記憶體佈局做 cache 內聯
> 優化(消滅 dependent pointer-chase),**不是** netlist→邏輯抽取的 IR。我們在「S2 階段」(目錄/fork
> 名),但**尚未進入真正的 IR 工作**。意義:這是「先用最小、低風險的工程手段過硬閘門」,把 S1 推到更高
> 起點,之後若要再攻 IR,門檻也更高(且 IR 必須維持事件驅動 —— batch IR 是已證實死路)。

### 工作流原則(2026-05-31 使用者定)

> **S2 目錄/fork = IR 階段專用。** 任何 **switch-level(非 IR)的 S1 優化**,即使是在 S2 探索出來的,
> 都 **sync 回 S1 原目錄**(`src/AprVisual.S1/` + `experiment/rust-s1/`);**switch-level 優化此後直接在
> S1 做**,S2 平時保持 == S1,只在真正動 IR 時才分歧。

**S2-A graduation**:既然 S2-A 是 switch-level 優化,已 sync 回 S1:
- ✅ **C# `src/AprVisual.S1/`**:4 個引擎檔複製自 S2,bit-exact `0x794A43ABDF169ADA`,~82–83K hc/s。
- ⏳ **Rust `experiment/rust-s1/`**:移植中。
- **新的 S1 門檻**:S2-A graduate 後,未來「打敗 S1」的基準從 ~77K 上移到 **~82K hc/s**(`perf/01` 記的是
  graduation 前的舊 baseline)。
