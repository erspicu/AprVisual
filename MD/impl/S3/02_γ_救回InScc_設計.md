...
- firing 25（γ ② part 2）：實作 **γ.1 通用 size-2 SCC solver**（兩步代換）—— InScc 5183→**899**、DrivingCovered 7340→**11624（78.9%）**、SCC 2254→**112**、等價 0 mismatch（checking + driving + 另一顆 ROM）。**下個 firing（γ ② part 3）：γ.2 = Stage B Pass-transistor Latch Extraction** —— 對剩下的 112 個 SCC（最大 496 = APU DMC down-counter tangle、其餘 ≤34 + 56 size-1 self-loop）找被 pass-transistor 驅動的 node、切斷 + `Prev()`、重抽 `Mux(pass_ctrl, pass_data, Prev(latch))` → SCC shatter。目標 coverage ~99%+（消滅 >20-node SCC）。先寫設計丟 Gemini（γ.2 是上次 Gemini review `..._152404.txt` 已給了大方向；可能直接做、或快速 review 一下實作細節）。
