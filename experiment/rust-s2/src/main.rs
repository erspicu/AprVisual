// wire_s2 — AprVisual.S2 Rust fork (verbatim wire_s1/S1 copy; S2 IR-stage starting point).
// bench-hc / shot / frame-dump runner; the inherited S1 engine is the bit-exact golden reference.
//
// Usage:
//   wire_s2 bench     <snapshot.aprsnap> <hc_count> [log_dir]
//   wire_s2 shot      <snapshot.aprsnap> <frames>   <out.png>
//   wire_s2 framedump <snapshot.aprsnap> <frames>   <out_dir>
...
fn usage() {
    eprintln!("usage: wire_s2 bench     <snapshot.aprsnap> <hc_count>");
    eprintln!("       wire_s2 shot      <snapshot.aprsnap> <frames>   <out.png>");
    eprintln!("       wire_s2 framedump <snapshot.aprsnap> <frames>   <out_dir>");
    eprintln!();
    eprintln!("Fast-path is always on in the S2 fork (inherited from S1; no flag).");
}
...
fn framedump(args: &[String]) {
    ...
    eprintln!("# wire_s2 (Rust S2 framedump): loading {path} ...");
...
fn bench(args: &[String]) {
    ...
    eprintln!("# wire_s2 (Rust S2 fork): loading {path} ...");
    let snap = snapshot::load(path).expect("snapshot load failed");
...
fn shot(args: &[String]) {
    ...
    eprintln!("# wire_s2 (Rust S2 shot): loading {path} ...");
    let snap = snapshot::load(path).expect("snapshot load failed");
    let clock_node = snap.clock_node;
    let vblank_node = snap.ppu_vblank_node;
    let mut wc = wire::WireCore::from_snapshot(snap);
