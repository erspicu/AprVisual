...
namespace AprVisual.Test
{
    ...
    internal static class TestRunner
    {
        ...
        // S4.6 bonus — dump the Verilog module (IR → RTL).
        private static int DumpEmittedVerilog(string outPath)
        {
            try { WireCore.ComposeSystem(chrIsRam: false, isTestRom: true); }
            catch (Exception ex) { Console.Error.WriteLine($"compose failed: {ex.GetType().Name}: {ex.Message}"); return 2; }
            AprVisual.Sim.Logic.IrEngine.Build();
            var v = AprVisual.Sim.Logic.IrEngine.EmitVerilog();
            int lines = v.Count(c => c == '\n');
            Console.Error.WriteLine($"# emitted Verilog: {AprVisual.Sim.Logic.IrEngine.DrivingCoveredCount} combinational + {AprVisual.Sim.Logic.IrEngine.ResidualSccNodes} SCC + {AprVisual.Sim.Logic.IrEngine.BusNodes.Length} bus nodes; {lines} lines, {v.Length} chars (best-effort — see the module header for the approximations)");
            if (outPath == "-") Console.Out.Write(v);
            else { File.WriteAllText(outPath, v); Console.Error.WriteLine($"# written to {outPath}"); }
            return 0;
        }
