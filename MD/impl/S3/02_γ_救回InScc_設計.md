...
- **γ.2 = Topological-Loop Breaker（targeted SCC back-edge cut）** —— firing 27 拿到 Gemini 的明確演算法（log `..._155332.txt`）：
  ...
  - **實作（firing 28）**：在 `IrEngine.BuildEvalOrder` 的 γ.1 迴圈裡加 `Gamma2BreakLoops()`（對每個還在 SCC 的 comp、把 SCC-internal 的 `NodeRef(latch)` 全換成 `Prev(latch)`）+ `Drive` field（存 DriveAnalysis 結果）。**結果：過度切（over-cut）→ disabled**。第一版 `LatchLike` 用「NextExpr 含 `Hold(v)`/`Prev(v)` 自我引用」→ 把 domino/precharge node 也當成 latch → 切 `cpu.#(A+B)4` 的 ALU carry edge → checking 0→49 mismatch（`prev(n9890) ≠ node(n9890)`）。第二版 `LatchLike` 改成 `di[v].Passes>0 && PullDown==null && PullUp==None && 至少一個 pass port 的 gate 是「clock-like」（gate ≥ 6 個 pass transistor）` —— SCC 843→22（dissolve 了大部分：chroma ring / hvtog / APU counter / sprite eval 都對），但**還是過度切** 6502 R/W-timing 邏輯（`cpu.notRnWprepad` 讀 `n9879`、`prev(n9879) ≠ node(n9879)`）+ 一個 palette-RAM cascade（`pal_ram_08_b4`）→ checking 8 mismatch、driving mismatch。**根因**：clock-fanout heuristic 不夠精準 —— 它把「某些 dynamic transmission-gate latch（半週期內會變、consumer 真的需要 current 值）」也當成「synchronous register（半週期內穩定）」。需要更精準的 register-boundary 分析（例如：只切「真的是 back-edge 且 target 確實在半週期內穩定」的邊；或判斷 gate 是否在 clock tree 的高層；或只切「latch → 純組合 detector」的邊、不切「latch → latch」carry/forward 的邊）。`Gamma2Enabled = false`（保留 code、留待 refine）。**下個 firing：refine γ.2** —— 先看 clock-fanout 分布（pclk0/pclk1/CPU clock 各 gate 幾個？control 訊號 gate 幾個？）決定能不能用更高的 threshold 區分；或改成只切 DFS back-edge（minimal）；或丟 Gemini 看這個 over-cut 怎麼解。
...
- firing 27（γ ② part b = γ.2 ①）：加 `--dump-scc` 的 per-SCC node detail 診斷 → 看清剩 56 個 SCC → 寫 γ.2 演算法 + 丟 Gemini（`..._155332.txt`）。
