...
The original plan was a four-stage pipeline (S1 switch-level engine → S2 netlist→IR → S3 CPU proof → S4 codegen + GPU) to push the simulation toward real time. We built and verified those stages — and found the counter-intuitive result that the "obvious" abstractions (**IR + codegen, or a GPU kernel**) ended up **slower** than the direct switch-level interpreter (code bloat, lost timing/correctness, algorithmic redundancy in batch re-evaluation). Real time is **~302× out of reach** and known-unreachable via this route.
...
Once the engine was fast enough to be a tool, we pointed it at the standard NES accuracy batteries: **all 147 NROM/CNROM NTSC ROMs** in the `nes-test-roms` collection (blargg's suites and friends — every mapper 0/3 test in the set, controller tests included), judged **unattended, once per simulated frame** — the blargg `$6000` protocol, ROM-requested soft resets (the engine pulses the console's reset line mid-run), terminal screen text decoded straight from the nametable, an on-screen CRC, or a scripted controller (`--input "A:2,Start:6.5"`, AprNes-compatible) feeding a behavioral joypad through the board's real LS368 path. Frame budgets are calibrated by an instrumented copy of our behavioral emulator [AprNes](https://github.com/erspicu/AprNes) acting as a fast oracle; the batch runs on core-pinned parallel workers, **longest tests scheduled first** (LPT, to minimise makespan) — a full 147-ROM sweep takes **6.2 h** on 7 pinned cores. (CNROM arrived as a behavioral CHR-bank latch, the controllers as a behavioral CD4021 — the 2A03/2C02 netlists stay untouched.)

**Result: 146 PASS / 1 FAIL / 0 timeouts on the clean full-regression run (99.3%)** — the score climbed from 115/26 to **146/1**, and that climb produced its own physics. Most FAILs were genuine gaps in our model: a two-state quiescent-settle engine cannot express some correct behaviors of the two dies — a same-half-cycle race, an unmodeled cross-die `$2001` write delay (`10-even_odd_timing`), an OAM-DMA bus-hold detail (`ppu_read_buffer` #67) — so for each we **supply the missing spec through a documented, test-mode behavioral shim** and the test PASSES. Several are **analog-race shims** (a DMC IRQ latch whose data must win a same-half-cycle race against its closing clock — emu-russia's independently reverse-engineered APUSim fails the same test the same way; ALU input latches that must *hold* against a collapsing bus; the LXA `$AB` ratioed analog bus fight). Every shim is **test-mode only and now global** (per Gemini's "no per-test overfitting" principle) — the engine's default benchmark path is bit-identical with or without them. The full taxonomy, fix table and methodology live in the bilingual **[test-fix knowledge base](MD/testrom/00_test-fix-knowledge-base.md)**.

**[Interactive report →](https://erspicu.github.io/AprVisual/Report/)** — per-test screenshots, detection method, frame counts, per-test khc/s, aggregate throughput stats, the exact hardware model (RP2A03G + RP2C02G die tracings; what is transistor-level and what is behavioral), split into two dossiers: *where the behavioral layer supplies the missing spec (these PASS)* and *faithful deviations (where failing IS the faithful result)*.

The **single remaining FAIL is the opposite case** — a failure *of faithfulness*. `cpu_dummy_writes_oam` declares its own prerequisite ("OAM reads must be reliable") holds on emulators **but not on the real NES**, and our pinned **RP2C02G** is exactly the PPU revision with the OAMADDR-write corruption bug: a behavioral emulator passes with a plain OAM array, while the switch-level 2C02, whose OAM is physical DRAM cells, fails the way real G silicon does. Forcing a PASS there would move the simulation *away* from the machine we target. Deep-dive Q&A: **[EN](MD/testrom/2026-07-05-faithful-deviation-qa.en.md)** · **[繁中](MD/testrom/2026-07-05-faithful-deviation-qa.md)**.
...
The shims work, but each is a black box: it says *what* to override, not *why the silicon behaves so*. **[S1a](https://erspicu.github.io/AprVisual/s1a.html)** ("advance & analogy") is a research fork (`src/AprVisual.S1A/`) that studies the ~1.1% genuinely-analog residue and replaces shims, one at a time, with **physically-grounded mechanisms** — charge storage & decay, drive-strength arbitration, RC propagation delay, transparent latches, board-level parts — each derived from the die geometry the engine already parses and each held to the same anchors (the S1 golden checksum, AC 141 + the 147 regression). As of **2026-07-22** those mechanisms are **integrated and always-on**: S1a is unconditionally the full engine in *every* mode (no `--no-shims`, no per-mechanism toggles — the raw switch-level engine is the separate S1 fork), and it clears both batteries on the merged engine — **AC 141/141 · 147 146/1**, re-verified bit-exact after a −375-line dead-code cleanup (its own benchmark checksum `0x41244C26C45EDD32` unchanged throughout). The honest map of which shims a mechanism can retire, which are undecidable in isolation, and which hit a genuine ceiling (e.g. open-bus last-byte lives in the *board's* bus capacitance, off either die) is itself the deliverable. Write-ups land on the site as each mechanism is built.
...
| `src/AprVisual.S1A/` | The **S1a** research fork — the unconditionally full-armed engine (all M1–M6 physically-grounded mechanisms always on, no opt-out; charge/strength/delay/latch/board); a study of the netlist's analog boundary. Clears AC 141/141 · 147 146/1. See `WebSite/s1a.html`. |
