...
|---|---|---|---|---|---|
| 1 | **時脈相位對齊(✅ 已完成 2026-07-04,淨 +7 兌現)**:`--phase-probe` 實測發現 **CPU ÷12 自由運轉、PPU ÷4 在 /res 釋放時重啟** → `--reset-hold-extra K` 可掃過全部 4 種真機對齊(相對相位 {1,7,5,3} ↔ K={0,1,3,5})。實測矩陣:NMI 邊沿家族 8 個在對齊 {7,5} 過、{1,3} 掛;`10-even_odd_timing` 恰好相反(只在對齊 1 過)—— **四種對齊無一全滿足,與 NESdev 記載的真機行為一致**(even_odd 系列本來就看上電對齊,實機要重開機碰運氣)。定案:runner 預設 `--reset-hold-extra 1`(對齊 7),收復四連星 4 + 舊版三連 3 + nmi_and_brk 1;`10-even_odd_timing` 改列**對齊相依的忠實偏差**(有完整四對齊矩陣佐證);sprdma×2 排除相位、確定歸 E 類。引擎預設仍 0,benchmark/golden checksum 不動 | 淨 +7 ✅ | 中 | 低 —— 已文件化 | **完成** |
