...
| 2026-05-12 | `<pending>` | β.0（WIP，有 channel-fanout bug） | 實作了：`WireCore.ProcessQueueWith(Action<int>)`（= ProcessQueue 的 copy、每個 enqueued node 用 callback 重算；S1 的 ProcessQueue/hot-path 不動）；`IrEngine.UseEventDriven` flag + `RecalcNodeForEventDriven(nn)`（IR-covered 的 nn → `EvalExpr(NextExpr[nn])` + `SetNodeState` + callback enqueue；其餘 → `WireCore.RecalcNode`（S1 group walk）；改變後 enqueue `irFanout[nn]`）+ `StepOneEventDriven()`（snapshot PrevStates → DeferRecalc handlers → `ProcessQueueWith(RecalcNodeForEventDriven)` → trace; Time++）+ `BuildIrFanout()`（CSR：`irFanout[v]` = NextExpr 引用 Node(v) 的 IR node 們）；`StepOneDriving` 在 `UseEventDriven` 時 dispatch 到它；`TestRunner --engine event`。`dotnet build` 0/0（Debug+Release）、`--selftest` 全 PASS、`--trace`（預設 S1）跟 HEAD 一致、`--trace-cmp --engine ir`（batch IR，`UseEventDriven` 預設 false）0 mismatch —— **預設 path 不受影響**。**但 `--trace-cmp --engine event` ~326K mismatch / 1353 distinct node**（第一個 t=6 `ppu.spr_d7_int#193`(hybrid)）—— **bug**：β 的「per-node EvalExpr 取代 S1 的 group walk」丟失了 group walk 的 side-effect（`RecalcNode(c)` 會 `ComputeNodeGroup(c)` + set 整個導通 group 的 member）。IR node `c`（如 sprite RAM cell）改變時，β 只 wake (a) `SetNodeState` 的 gate-fanout + (b) `irFanout[c]`（引用 c 的 IR node）—— MISSES (c) **channel-fanout**（透過導通 pass 跟 c 連在一起的 non-IR node（hybrid bus / SCC），S1 的 group walk 隱式處理）→ stale → 錯。`UseEventDriven` 預設 false（opt-in），不影響任何既有東西。**下個 firing**：修 channel-fanout（need `WireCore` 的 per-node channel-transistor list；或重新考慮 β formulation —— 也許 IR node 也走 `WireCore.RecalcNode`（group walk），只在「group 是 1-node 的純-IR 葉子」時用 EvalExpr 快路；或 hybrid 模式）。|
