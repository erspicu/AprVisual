...
namespace AprVisual.Sim.Logic
{
    ...
    internal static unsafe class IrEngine
    {
        ...
        public static bool EnableBusLowering = false; // S4 γ.4 (WIP, disabled): gives the hybrid bus nodes a wired-AND pseudo-NextExpr → IR coverage ~99%, BUT (a) a couple of rare checking-mode mismatches (the wired-AND "Other is 0" approximates "Other is strongly pulled to 0"; handler-driven nodes in u1/u4/cart are now skipped), (b) the buses are so mutually pass-coupled that modelling them folds ~10K nodes into one giant SCC (driving-mode coverage 79%→~31%; fixed-K (S4.3) would handle it but K may be large). Set true to A/B-test it.
