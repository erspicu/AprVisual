...
| 階段 | 做了什麼 | 狀態 |
|---|---|---|
| **S1** | C# 重寫 MetalNES 的 switch-level event-driven 引擎（`WireCore.*`：`.js` 模組 parser、module instancing、`recalcNodeList`/`processQueue`/`recalcNode`/`computeNodeGroup`、256-entry group-resolution LUT、per-cycle handler chain、行為式 RAM/ROM、系統載入、trace）。算法 = `ref/metalnes-main` 的 `wire_compute`（本身是 Visual6502 `chipsim.js` 的最佳化 port）—— 我們忠實 port，加上幾個 fidelity refinement（雙 pull 極性 / weak-depletion flag / 顯式 lowering / driven-vs-held 可分辨）+ 從零的 C# 實作（含 ~MetalNES 級速度的 unmanaged 熱路徑）+ 把它結構化成 S2–S4 的可驗證底座。**「S1 裡哪些是 ported、哪些是我們的」的詳細釐清見 `MD/impl/S1_貢獻分析_vs_MetalNES.md`。** | **DONE**、過 blargg、~47K hc/s（~14s/幀 ≈ 0.07 FPS）。**永久的離線 golden reference。** |
