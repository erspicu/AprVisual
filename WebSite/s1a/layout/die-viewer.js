...
(function () {
  ...
  DieViewer.prototype._draw = function () {
    ...
    // visible die-rect (for culling), with a small margin
    var d0 = this._toDie(0, this.vpH), d1 = this._toDie(this.vpW, 0);   // note Y flip: (0,vpH)->min, (vpW,0)->max
    var vminx = d0[0], vmaxx = d1[0], vminy = d0[1], vmaxy = d1[1];
    var segs = this.L.segs, bb = this.bboxes, self = this;

    // pass 1: backdrop, by layer colour
    for (var i = 0; i < segs.length; i++) {
      var box = bb[i];
      if (box[2] < vminx || box[0] > vmaxx || box[3] < vminy || box[1] > vmaxy) continue;   // cull
      var s = segs[i];
      ctx.fillStyle = LAYER_COLORS[s[1]] || "#2a2a33";
      this._path(ctx, s);
      ctx.fill();
    }
    // pass 2: highlights on top, by category (drawn thicker so they read when zoomed out)
    if (this.hl) {
      var cats = this.hl.categories;
      Object.keys(cats).forEach(function (key) {
        if (!self.activeCats[key]) return;
        var c = cats[key];
        ctx.fillStyle = self._rgba(c.color, 0.72);
        ctx.strokeStyle = self._rgba(c.color, 0.95);
        ctx.lineWidth = Math.max(0.6, Math.min(2, self.view.scale * 40));
        c.nodes.forEach(function (n) {
          var idxs = self.segsByNode[n]; if (!idxs) return;
          for (var j = 0; j < idxs.length; j++) {
            var box2 = bb[idxs[j]];
            if (box2[2] < vminx || box2[0] > vmaxx || box2[3] < vminy || box2[1] > vmaxy) continue;
            self._path(ctx, segs[idxs[j]]); ctx.fill(); ctx.stroke();
          }
        });
      });
    }
  ...
  DieViewer.prototype._requestDraw = function () {
    if (this._raf) return;
    var self = this;
  ...
  DieViewer.prototype._wire = function () {
    ...
    window.addEventListener("mouseup", function () { drag = false; vp.classList.remove("dv-grab"); });
    ...
    vp.addEventListener("wheel", function (e) {
      ...
      self.view.scale = ns; self._requestDraw();
    }, { passive: false });
