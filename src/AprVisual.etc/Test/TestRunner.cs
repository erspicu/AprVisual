...
namespace AprVisual.Test
{
    ...
    internal static class TestRunner
    {
        ...
        public static int Run(string[] args)
        {
            ...
            string? exportEngineFull = null;// --export-engine-full <path>: dump the FULLY-optimized engine for the C++ ours-full port
            string logDir = "log";
            int maxWait = 15;
            int traceCycles = 64;
            int shotFrames = 3;
            ...
            for (int i = 0; i < args.Length; i++)
            {
                switch (args[i])
                {
                    ...
                    case "--export-engine-full": if (i + 1 < args.Length) exportEngineFull = args[++i]; break;  // dump fully-optimized engine for the C++ ours-full port
            ...
            if (exportEngineFull != null) return WireCore.ExportOursEngineFull(cpuBenchDir ?? $"src/AprVisual.etc/netlists/{cpuChip}", cpuChip, exportEngineFull);
