...
namespace AprVisual.Sim.Logic
{
    ...
    internal static class GpuCodegen
    {
        ...
        public static string EmitHlslCompute()
        {
            ...
            //   step()   = the DAG eval only (a drop-in for StepOneDriving's step-4; the CPU sets up PrevW).
            //   gpuRun() = NumHc half-cycles all on the GPU (each: snapshot PrevW<-NodeState; DAG eval) — for
            //              the no-CPU-round-trip speed benchmark (G.a). NOTE: it does NOT do the SCC fixed-K /
            //              bus resolver / memory handlers, so the simulation state goes garbage after the first
            //              half-cycle — but the per-half-cycle COMPUTE (the ~11680-node DAG eval, ~80% of the
            //              work) is roughly right, which is what the speed measurement needs.
            return
$@"// auto-generated by GpuCodegen.EmitHlslCompute -- S4.6 GPU step kernels (bytecode interpreter).
// {IrEngine.EvalOrder.Length} EvalOrder nodes in {NumLevels} topo levels; {sccNodes} SCC nodes in {NumSccs} SCCs (not used here);
...
RWStructuredBuffer<uint> PrevW     : register(u1);   // prev-half-cycle node values (Hold/Prev read this); gpuRun() snapshots it
StructuredBuffer<uint>   Bytecode  : register(t0);
StructuredBuffer<uint>   BcOff     : register(t1);
StructuredBuffer<uint>   BcCount   : register(t2);
StructuredBuffer<uint>   NodeId    : register(t3);
StructuredBuffer<uint>   LevelStart: register(t4);
cbuffer Params : register(b0) {{ uint NumHc; uint NNodes; uint Pad0; uint Pad1; }};
...
{{
    ...
    {{
        ...
        else if (op == 1u) {{ GStk[sb + sp] = PrevW[arg]; sp += 1u; }}
...
void step(uint tid : SV_GroupIndex) {{ DagEval(tid); }}

[numthreads(NUM_THREADS, 1, 1)]
void gpuRun(uint tid : SV_GroupIndex)
{{
    [loop] for (uint hc = 0u; hc < NumHc; hc++)
    {{
        for (uint i = tid; i < NNodes; i += NUM_THREADS) PrevW[i] = NodeState[i];
        AllMemoryBarrierWithGroupSync();
        DagEval(tid);
    }}
}}
