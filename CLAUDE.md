...
- **TODO / after S4** (no `/loop` — do these on request): (1) ~~the explicit LLVM `-O3`~~ DONE (firing 18 — `LlvmCodegen.TryRunO3` via `LLVM.RunPasses`); (2) **S4.6 — GPU compute shader** (ComputeSharp/D3D12-HLSL, headless; one workgroup + groupshared state + `GroupMemoryBarrier` between DAG topo levels + N half-cycles/dispatch + memory handler on-GPU; `--gpu-trace-cmp`/`--gpu-frame` planned; SKSL/AprNesAvalonia is *not* suitable for the compute — fragment-only — only for a CRT-display shader if a UI is added later) — full plan: `MD/impl/S4/01_S4.6_GPU_compute_設計.md`; (3) a **`cpu-opt` branch** forked ~end-of-S3 — an *event-driven* IR interpreter (dirty-set, re-eval only the nodes whose inputs changed) — the actual path to fast single-instance (user deprioritized for now in favour of trying GPU first); (4) an HDL emit (`--dump-emitted-verilog` — the IR is essentially synthesizable RTL; use = Verilator / FPGA; ~1-2-day bonus) — see `MD/impl/S4/01` §8.
