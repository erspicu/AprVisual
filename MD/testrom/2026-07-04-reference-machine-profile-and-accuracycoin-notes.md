...
## 6. DMC #19 深究:排除法記錄(2026-07-04)

`7-dmc_basics` #19(「空 buffer 應立即填充」,fail 於第 31 幀)的嫌疑排除進度:

| 嫌疑 | 方法 | 結果 |
|---|---|---|
| DMC DMA 機能缺失 | `--rdy-probe`(逐 hc 數 RDY 翻轉) | **排除** —— 幀 1-16 每幀 ~138 次翻轉、每次停機 ≈3.5 CPU cycles(教科書值),取樣節奏正常 |
| netlist 轉錄誤差 | 對 QuietuST 上游原始檔做語義雙向 diff(名字解析後之 gate/c1c2/weak 多重集合) | **排除** —— 2A03:10,912=10,912、segdefs 3,373=3,373;2C02:16,872=16,872;**零差異** |
| Lowering(S1.5) | `--no-lower` A/B | **排除** —— 同碼同幀失敗 |
| RDY 焊墊未接線 | 板定義檢查 + rdy-probe | 未接屬實,但 DMA 停機正常運作 → 無礙(真 2A03 封裝亦未外露 RDY) |

**存活嫌疑**(依可能性):
1. **上游 Visual2A03 描繪 vs 真晶粒**(APU/DMC 區的微小描繪差)—— diff 摸不到;需 emu-russia/breaks 交叉或顯微 trace 比對 nesdev 文獻的 cycle 級行為。
2. **行為層子週期精度** —— 對 #19 作用面窄(鏈路幾乎全在矽內,行為 ROM 只供一個 byte);
   對 dma_2007 家族(DMA×PPU 匯流排交會)作用面厚 —— 家族可能拆成兩個根因。
3. prunes/fast-path —— 結構證明 + bit-exact 閘背書,列最後。

**下一步**:顯微 trace —— 跑到 #19 的 `$4015` 啟用瞬間,逐 hc 記錄 rdy/位址/資料匯流排與 APU 內部狀態,
對照 nesdev 的「啟用後 2-4 cycle 內首次取樣」時序,定位差異的第一個 cycle。
