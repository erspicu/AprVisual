# -*- coding: utf-8 -*-
# AprVisual S1 ARM (Pi 5 / Cortex-A76) per-version report — stock 2.4GHz vs overclock 3.0GHz + temp trend.
import csv, os, html
...
def load(fn):
    ...
    return d
stock=load("version_perf_arm.csv")          # 2.4 GHz
ocd=load("version_perf_arm_oc.csv")          # 3.0 GHz (+temp)
order=list(ocd.keys())
rows=[ocd[v] for v in order]
...
def dual_chart(s_vals,o_vals,w=920,h=290,pad=54):
    n=len(o_vals);allv=s_vals+o_vals;vmin=min(allv);vmax=max(allv);span=(vmax-vmin)or 1
    lo=vmin-span*0.10;hi=vmax+span*0.10
    ...
    def dots(vals,color,mark=False):
        s=""
        for i,v in enumerate(vals):
            big=mark and order[i] in MILE
            s+=f'<circle cx="{X(i):.1f}" cy="{Y(v):.1f}" r="{4.6 if big else 2.8}" fill="{"#ffd24a" if big else color}" stroke="#0d1420" stroke-width="{1.4 if big else 0}"><title>{order[i]}: {v:,}</title></circle>'
        return s
    grid=""
    for k in range(5):
    ...
    leg=f'<g><rect x="{w-205}" y="14" width="12" height="3" fill="#39d98a"/><text x="{w-189}" y="19" font-size="11" fill="#cdd9ec">3.0GHz 超頻</text><rect x="{w-205}" y="32" width="12" height="3" fill="#5b86c4"/><text x="{w-189}" y="37" font-size="11" fill="#cdd9ec">2.4GHz 原廠</text></g>'
    return f'<svg viewBox="0 0 {w} {h}" width="100%" style="max-width:{w}px">{grid}<polyline points="{poly(s_vals)}" fill="none" stroke="#5b86c4" stroke-width="2" stroke-dasharray="5 4"/>{dots(s_vals,"#5b86c4")}<polyline points="{poly(o_vals)}" fill="none" stroke="#39d98a" stroke-width="2.6"/>{dots(o_vals,"#39d98a",True)}{xl}{leg}</svg>'
...
def temp_chart(vals,w=920,h=180,pad=46):
    ...
    guard=f'<line x1="{pad}" y1="{Y(75):.1f}" x2="{w-pad}" y2="{Y(75):.1f}" stroke="#f06a6a" stroke-dasharray="6 4"/><text x="{w-pad}" y="{Y(75)-5:.1f}" text-anchor="end" font-size="11" fill="#f06a6a">75°C 中止門檻</text>'
...
chart=dual_chart([stock[v]["hc"] for v in order],[r["hc"] for r in rows])
...
for r in rows:
    ...
    cards+=f'<div class="vc" style="{star}"><div class="vh"><span class="mv">{r["v"]}</span> <span class="mut">{r["date"]} · {r["tf"]}</span> <b>{d[0]}</b></div><div class="vd">{html.escape(d[1])}</div><div class="vm mut">{r["hc"]:,} hc/s @3.0G {dhc} · 原廠2.4G {stock[r["v"]]["hc"]:,} · native {r["native"]:,} B · {r["temp"]}°C</div></div>'
    prev=r
trs=""
for r in rows:
    sc=r["hc"]/stock[r["v"]]["hc"]
    trs+=f'<tr><td class="mv">{r["v"]}</td><td class="mut">{r["date"]}</td><td class="num">{stock[r["v"]]["hc"]:,}</td><td class="num" style="color:#8fe9bd">{r["hc"]:,}</td><td class="num">{sc:.2f}×</td><td class="num">{r["temp"]}°C</td><td class="num">{r["native"]:,}</td><td class="mut">{r["tf"]}</td></tr>'
...
<title>AprVisual S1 — ARM (Pi 5) 效能歷史 + 3.0GHz 超頻</title><style>
 ...
 .hero .wrap{{padding-bottom:0}} h1{{font-size:25px;margin:0 0 6px;color:#fff}} h2{{font-size:19px;margin:38px 0 12px;color:#fff;border-bottom:1px solid #23314a;padding-bottom:6px}}
 ...
 .k .big{{font-size:28px;font-weight:800;color:#fff;line-height:1.1}} .k .lab{{font-size:12px;color:#7f93b3;margin-top:4px}} .grn{{color:#39d98a}} .org{{color:#f0a35e}}
...
<h1>🔥 AprVisual S1 — ARM 效能歷史(Raspberry Pi 5)+ 3.0GHz 超頻</h1>
<p class="mut">switch-level NES 模擬器 · 18 版(2026.05.30→2026.06.19)· Cortex-A76 · 全程 <b>bit-exact</b>(<code>0x9174E19D961CB6E5</code>)· best-3-of-5 · x64 版見 <a href="perf-history.html" style="color:#4ea1ff">perf-history.html</a></p>
<div class="kpi">
 <div class="k"><div class="big grn">{spd:.2f}×</div><div class="lab">引擎演進(首→末版 @3.0G)</div></div>
 <div class="k"><div class="big">{last['hc']:,}</div><div class="lab">峰值 hc/s @ 3.0GHz(06.19)</div></div>
 <div class="k"><div class="big org">+{(ocgain-1)*100:.0f}%</div><div class="lab">超頻 3.0G vs 原廠 2.4G(末版)</div></div>
 <div class="k"><div class="big">{tmin:.0f}–{tmax:.0f}°C</div><div class="lab">全程溫度(門檻 75°C 未觸)</div></div>
</div></div></div>
<div class="wrap">
<div class="note warn"><b>超頻 3.0GHz 全程穩定 + bit-exact。</b> Pi 5 原廠上限 2.4GHz;在 <code>config.txt</code> 設 <code>arm_freq=3000</code> 後,18 版全部位元完全相同(checksum 0x9174…),末版 <b>{stock[last['v']]['hc']:,} → {last['hc']:,} hc/s(+{(ocgain-1)*100:.0f}%)</b>,接近時脈比(2.4→3.0=1.25×)的<b>近線性</b>加速 → 證明此負載大多 core/L1-bound、非 DRAM-bound。<b>溫度全程 {tmin:.1f}–{tmax:.1f}°C(主動散熱),完全沒有上升趨勢,離 75°C 中止門檻與 85°C 降頻點都很遠。</b></div>

<h2>📈 吞吐量:原廠 2.4GHz vs 超頻 3.0GHz(hc/s · ★=里程碑)</h2>
<div class="card">{chart}</div>
<div class="card">{bars}</div>

<h2>🌡 溫度趨勢(每版測完量一次 · 3.0GHz)</h2>
<div class="card">{tchart}</div>
<div class="note"><b>溫度持平在 ~{tmin:.0f}–{tmax:.0f}°C</b>,18 版連續燒機(~14 分鐘滿載)<b>沒有累積升溫</b>。最高 {tmax:.1f}°C、零 throttle(<code>get_throttled=0x0</code>)。安全 guard:測完任一版若 &gt;75°C 自動中止 —— 本次未觸發。</div>

<h2>🛠 每一版改了什麼(數值 @3.0GHz)</h2>
{cards}

<h2>📋 完整數據(原廠 vs 超頻)</h2>
<div class="card" style="overflow-x:auto"><table>
<tr><th>版本</th><th>日期</th><th class="num">2.4GHz</th><th class="num">3.0GHz</th><th class="num">超頻比</th><th class="num">溫度</th><th class="num">native(B)</th><th>TFM</th></tr>
{trs}</table></div>

<h2>🔬 方法論與限制</h2>
<ul>
<li><b>機器</b>:Raspberry Pi 5(Arm Cortex-A76, ARMv8.2, 4c, 主動散熱)。原廠 2.4GHz / 超頻 <code>arm_freq=3000</code>(3.0GHz)兩組都 best-3-of-5、同 400k hc、cpufreq <b>performance governor 鎖定</b>。</li>
<li><b>超頻設定</b>:<code>/boot/firmware/config.txt</code> 加 <code>arm_freq=3000</code>(原始已備份 <code>config.txt.bak_oc</code>;還原 = 移除該行 + reboot)。2.8GHz→3.0GHz 逐級驗證,每級先確認 bit-exact + 溫度/throttle 才升。</li>
<li><b>安全</b>:每版測完量溫度,&gt;75°C 即中止(本次最高 {tmax:.1f}°C,未觸發)。零 throttle。</li>
...
<li>vs x64:見 <a href="perf-history.html">x64 (Zen2) 報告</a>(末版 ~138.7K)。超頻後 ARM 末版 {last['hc']:,} ≈ Zen2 的 {last['hc']/138708:.2f}×。</li>
</ul>
<p class="mut" style="margin-top:26px">ARM cycle-level 紀錄 · 全程 bit-exact · 2.4GHz 原廠 + 3.0GHz 超頻 + 溫度趨勢 · 產生於 2026-06-19 · Pi 5 主動散熱</p>
</div></body></html>"""
p=os.path.join(OUT,"perf-history-arm.html")
open(p,"w",encoding="utf-8").write(HTML)
print("WROTE",p,f"({len(HTML)}B) engine {spd:.2f}x  OC last {last['hc']} (+{(ocgain-1)*100:.0f}% vs stock)  temp {tmin}-{tmax}C")
