...
- **S4.6 — GPU compute shader**: **explored; the codegen + the equivalence-verified GPU step are done; the fast (all-on-GPU) version is parked.** D3D11 compute via Vortice (`Vortice.Direct3D11` + `Vortice.D3DCompiler` NuGet, headless, no window — `--gpu-spike` verified on the RTX 4080). The IR's EvalOrder is topo-layered (`IrEngine.BuildTopoLevels` — 44 levels, ~265-wide avg; `--dump-topo-levels`) and codegen'd into an HLSL bytecode-interpreter kernel (thread = one node, runs that node's RPN bytecode, `AllMemoryBarrierWithGroupSync` between levels; `GpuCodegen.EmitHlslCompute` / `--dump-emitted-hlsl`). The GPU step (the DAG eval, a drop-in for `StepOneDriving`'s step-4 with S1 still doing step-5) is equivalence-verified (`--trace-cmp --engine ir --gpu-step` 0 mismatch). **Benchmark**: ~264 s/frame ≈ 2475 hc/s (with the per-step round-trip; `--gpu-step`) — ~18× slower than S1. And the *round-trip-free* lower bound (`--gpu-bench` — `gpuRun()` does `N` half-cycles of the DAG eval all on the GPU, no round-trip, no SCC/bus/handlers): ~155 s/frame ≈ ~4225 hc/s — still **~10.7× slower than S1** (~14.5 s/frame). So "one NES on GPU" is a dead end: one workgroup uses ~1/76 of the RTX 4080's SMs, and the bytecode interpreter's opcode dispatch is 8-way warp-divergent. The equivalence-clean all-on-GPU version is moot anyway (it'd hit the same within-half-cycle-event-sequencing obstacle as the parked ping-pong run-path — the PPU precharged-dynamic readout — which a "2-phase precharge model" only partly addresses; bumping the ping-pong's `K_OUTER` 4→24 didn't move the ~322K mismatches). The GPU's strength is *throughput* (many instances — bit-sliced — would win overwhelmingly; the bit-sliced emit exists), not single-instance latency. SKSL/AprNesAvalonia is *not* suitable for the compute (fragment-only). Plan + the firing log: `MD/impl/S4/01_S4.6_GPU_compute_設計.md`.
- **2026-05-13 — project wound down (the user's call).** A bounded experiment (`MD/impl/S4/00` §14) measured S1 ≈ 47K hc/s vs the IR-driving runtime ≈ 7K (C#-JIT step-4) / ≈ 16K (LLVM-MCJIT step-4) — i.e. **every AOT-batch backend is 3–6× *slower* than S1**, and the math shows folding the SCC/bus blocks into the LLVM step (or a perfect Verilator-style single-pass model) can't flip that: a batch model re-evaluates ~14.7k nodes/half-cycle when only ~hundreds change, so it's algorithmically ~50× more work than S1's dirty-set; codegen quality only buys ~5–10× cheaper-per-eval. The one thing that *could* beat S1 on CPU is the event-driven β (re-eval only the dirty — the `cpu-opt` branch), but it has an unsolved correctness issue (per-node `EvalExpr` breaks the conducting-group atomicity that S1's group-walk gives; parked at `cpu-opt` `11a01d3` with ~425K mismatches) **and even at its ceiling (~2–3×, per Gemini) it doesn't reach real-time** (~840× faster than S1 needed). So: the `MD/struct` goal "single-instance correct + real-time-usable" — **correctness is done; "real-time" is not reachable via the switch-level→logic-IR pipeline** (that needs a non-transistor behavioral emulator — `ref/AprNes` itself — a different project). The value here is and remains the *translation pipeline* + correctness, which is complete. Parked artifacts (γ.2, γ.4 `BusLowering.cs`, the ping-pong run-path, the bit-sliced/HLSL/Verilog emit, the `cpu-opt` β branch) are kept behind flags / on the branch as "this was explored; here's where the boundary is". S1 = the permanent offline golden reference. Don't re-start the work without a new explicit ask.
- **Earlier TODO / after-S4 ideas** (now superseded by the wind-down, kept for reference): the `cpu-opt` event-driven branch; the all-on-GPU version with a 2-phase precharge model; `--dump-emitted-verilog` (the IR → synthesizable RTL — Verilator / FPGA; `MD/impl/S4/01` §8) already exists.

Per-node equivalence gate between stages. Scope: 2A03 + 2C02, NROM only. (An autonomous `/loop` cron drove firings 13–29 of S3; the user stopped it at firing 30 to consolidate docs. The S4 + S4.6 + cpu-opt-β work was likewise `/loop`-driven; all those crons were CronDelete'd as each line wound down — don't re-create them.)
