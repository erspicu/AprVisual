...
- **TODO / after S4** (no `/loop` — do these on request): (1) the **`cpu-opt` branch** forked ~end-of-S3 — an *event-driven* IR interpreter (dirty-set, re-eval only the nodes whose inputs changed) — the actual path to fast single-instance; (2) the all-on-GPU version (the big effort above, with the 2-phase precharge model) if someone wants to push S4.6 further; (3) `--dump-emitted-verilog` already exists (the IR → synthesizable RTL — Verilator / FPGA; `MD/impl/S4/01` §8).
