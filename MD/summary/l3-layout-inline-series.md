...
> Branch:`aot-codegen`
> 範圍:WireCore L3 hot data layout 緊密化 + hot method JIT/LLVM 強制 inline + iterative BFS + handler batch settle + no-op skip
> 工程量:13 commits、~200 行 code(declaration / cast / annotations / iterative loop / batch settle)、0 algorithm 變動
> 結果(`--fast-path` 開啟,真實 peak):**C# +29.3% best(37,928 → 49,055 hc/s)、Rust +10.2% best(48,622 → 53,565 hc/s)**
...
## 對齊比較數字(2026-05-28 終態,6d01abe + 5c6ab2b)

| Mode | C# best | C# avg | Rust best | Rust avg | Rust > C# |
|---|---|---|---|---|---|
| default | 48,133 | 47,054 | 50,686 | 50,573 | +5.3% |
| + `--fast-path`(peak) | **49,055** | **48,036** | **53,565** | **53,247** | **+9.2%** |
| + `--prune-merge` | 45,352 | 45,008 | (-6.7% 略測) | — | — |
| + `--fast-path --prune-merge` | 47,356 | 46,852 | (combo 不贏 fp 單獨) | — | — |

C# 從 inline series 之前的 37,928 → **49,055** = **+29.3% best**(`--fast-path` peak)
Rust 從 inline series 之前的 48,622 → **53,565** = **+10.2% best**(`--fast-path` peak)

### `--prune-merge` 在 inline cascade 後反向

兩平台後 inline cascade,**`--prune-merge` 都變慢**:
- C# -5.8%(46.4K vs 48.1K default)
- Rust -6.7%(memory `[[rust-port-best-config]]` 紀錄)

跟 historical memory `[[math-algos-branch-charter]]` 紀錄的 C# `--prune-merge` Phase 1 +1.37× **完全相反**。 原因:那時 group walk 本身慢(沒 inline,沒緊湊 L3 layout),prune-merge skip 真實省下大 chunk。 現在 group walk 已被 inline cascade 壓 flat,skip 沒省到關鍵,只剩 NodeGroupIDs 維護成本。

memory `math-algos-branch-charter` 的 +1.37× 是 stale baseline ── 跟現在 codebase 不再適用。
...
- 當前 best:Rust 53,565 hc/s(`--fast-path`)
- NES real-time:17,873,200 hc/s(NTSC 21.477 MHz / 24 × 2 half-cycles/cycle ≈ 1.79M CPU cycle/s × 10 hc/CPU cycle 大致 = 17.87M)
- 距離:**~334×**

跟 inline 系列之前(~366×)比,gap 收窄一些。 single-instance CPU 上仍**遠不可達 real-time**。 memory `[[s4-route-single-instance]]` 的結論不變:此 pipeline 上 real-time 需要算法改變(behavioral / cycle-accurate)或 multi-instance bit-sliced GPU(S4 已實作 PoC)。

## Commit history(最後 13 個)

| Commit | 變動 |
|---|---|
| `e6824e5` | C# _inGroup int→byte |
| `35baec8` | C# _groupBuf int→ushort |
| `ed3e637` | C# TransistorList int→ushort |
| `d7111f1` | C# NodeInfo SoA split |
| `ca49aa7` | C# RecalcHash int→byte |
| `a357cb2` | Rust TransistorList i32→u16 |
| `4657199` | C# inline EnqueueNode + GetNodeValue |
| `eb89044` | C# inline SetNodeState |
| `a929ae0` | C# inline ComputeNodeGroup + RecalcNodeFast + RecalcNode |
| `af5d4f0` | Rust #[inline(always)] hot methods |
| `73b1f88` | C# iterative BFS + inline AddNodeToGroup(LLVM 上反向,不 sync) |
| `5c6ab2b` | Rust no-op skip + batch settle + pending_luts fix + decoder direct |
| `6d01abe` | C# same(SetHighQueued / WriteBits batch) |
