...
這幾件事如果不先講,接手的人會被誤導、或白做工。

### (1) main 上的 commit 訊息會騙你

`e5396da` 的訊息寫著 **"fixes the AccuracyCoin stack overflow"**。

**這只對一半。** 它修好了 stack overflow(崩潰),但**沒有**修好底層的病 —— 同一個 frame 4480
現在改成**無限空轉**。**AccuracyCoin 到今天仍然跑不完。**
如果你只讀 git log,你會以為這件事結案了。**它沒有。**

### (2) 有一份證據是我(前一手)自己弄丟的

無 guard 版的 `run.err.log` **原始檔已被覆蓋**。原因:2026-07-11 12:30 重啟長跑時,
PowerShell 的 `Rename-Item` 因目錄被佔用而失敗,但 `Start-Process` **仍然啟動了引擎**,
它的 `RedirectStandardError` 把舊 log 截斷覆寫掉了。

→ `evidence/noguard-stackoverflow.txt` 裡的那段堆疊是**轉錄的,不是原始檔**(該檔已註明出處)。
→ 但 `progress.jsonl` 是 append 模式所以**倖存**,可**獨立佐證**兩次都停在 frame 4480。
→ **教訓:重導向輸出到既有 log 檔前,先確認搬移/清理真的成功了。**

### (3) 「AccuracyCoin 141/141 全過」那個數字**不是 S1 跑出來的**

`AprAccuracyCoinUnattended/private/1.png` 那張 `TESTS PASSED: 141 / 141` 的截圖,
以及本專案任何提到 141/141 的地方,**都來自行為層模擬器 AprNes**,不是 switch-level 的 S1。

**S1 從來沒有跑完過 AccuracyCoin。** 它每次都死在 frame 4480(92%)。
→ **絕對不要把 141/141 當成 S1 的成績放上網站或論文。** 那會是不實陳述。

### (4) 官網的 AccuracyCoin 區塊是空的,而且應該保持空的

`WebSite/index.html` 的 §03·5 有一組 `<!-- AC-PANEL:BEGIN/END -->` 標記,目前**內容是空的**。
產生器是 `tools/testrom/ac_chart.py`(數據全部從執行結果算出,頁面上沒有手打的數字)。

**在 S1 真的跑完之前,不要注入資料。** 現在注入只會產生一個「測試進行中、統計不完整、沒有截圖」
的半成品區塊,而且 `WebSite/` 是從 main 直接部署的 —— **commit 就等於上線**。

### (5) 一個尚未驗證、但可能很關鍵的變因:`--ac-verdict` 關掉了 cart-extraram

`--ac-verdict` 會把 `cart-extraram` **關掉**(`TestRunner.Test.cs`)。這個決定本身是**對的** ——
AccuracyCoin 有數個測試在量 `$6000-$7FFF` 的 **open bus**,掛上 RAM 會讓那些讀取讀到 RAM,
測試照跑照給結果但**結果是錯的**(靜默錯誤)。

**但:這個震盪是否與「該區域為 open bus(浮接)」有關,完全沒有驗證過。**

- 若相關 → 修法方向可能完全不同(是浮接匯流排的處理,不是 $2007 本身)。
- 若無關 → 可排除一個大變因。

**這是最便宜也最該先做的一個實驗**(雖然仍要 6.9 小時):用 `--extra-ram` 強制掛上 RAM 再跑一次,
看它**還會不會**停在 frame 4480。⚠️ 注意:這樣跑出來的**測試結果是不可信的**(open-bus 測試會被污染),
但我們**只在乎「會不會 hang」**這一個位元的資訊,所以這個實驗依然成立。
...
### P0.5. 用一個實驗排除 / 確認 open-bus 變因

見 §0.5(5)。加上 `--extra-ram` 強制掛上 `cart-extraram` 再跑一次:

```bash
dotnet ...AprVisual.S1.dll --test AprAccuracyCoinUnattended/AccuracyCoin.nes \
  --ac-verdict --extra-ram \
  --max-frames 7305 --reset-hold-extra 1 \
  --progress-frames 10 --progress-dir <dir> --system-def-dir ...
```

- **還是停在 frame 4480** → open bus 無關,禍首確實在 `$2007` / PPU 資料埠這條路徑上。
- **不再停** → 震盪與「`$6000-$7FFF` 浮接」有關,修法方向要整個重想。

⚠️ 這一跑的**測試成績是不可信的**(open-bus 測試會被 RAM 污染),
但我們只要「會不會 hang」這一個位元,所以實驗仍然成立。
...
| 工具 | 用途 |
|---|---|
| `--progress-frames N --progress-dir DIR` | 引擎每 N 個模擬幀存一張截圖 + 一行狀態到 `progress.jsonl`(幀數 / 模擬秒 / hc / 牆鐘 / 秒per幀 / `Debug_EC`)。**就是它讓我們看見「卡住」而不是誤以為「很慢」。** 對模擬唯讀,預設關閉,benchmark 路徑不受影響。 |
| `tools/testrom/ac_watch.py` | 盯著長跑:每 600 幀寄信(附截圖)、跑完寄結果、**行程無聲死掉或 15 分鐘沒進展會寄警告**。 |
| `tools/testrom/ac_report.py` | 從 `progress.jsonl` + 結果 JSON 算出時間 / 吞吐量統計。 |
| `tools/testrom/ac_chart.py` | 產生官網的 AccuracyCoin 區塊(含效能曲線 SVG)。**在 S1 真的跑完前不要用**(見 §0.5(4))。 |
| DEBUG guard telemetry | DEBUG 版跑完會印 `# [guard] nested entries absorbed: total=… max-per-drain=…`。Release 編譯掉。 |
| `AprNesRef/` 的 AprNes | **快速 oracle**。`--dump-ac-results` 會 dump AccuracyCoin 的完成區塊與結果表。**§3 的禍首就是這樣查出來的** —— 秒級,不用等 switch-level。 |
| `dotnet-trace` | 已安裝。對**還活著的**卡住行程抓 CPU trace(§4.2 就是這樣做的)。**下次撞到,趁行程還活著先抓 trace,別急著砍掉。** |

### 目前世界狀態

- **沒有任何測試在跑**(卡住的行程已被砍掉)。
- guard 已在 **main**(`e5396da`)。黃金 checksum 不變,147 顆回歸不受影響。
- 官網 AccuracyCoin 區塊**空的**(標記在,內容無)——見 §0.5(4)。
...
1. **讀 §0.5(必須先知道的五件事)。** 尤其是「git log 會騙你」和「141/141 不是 S1 的成績」。
2. **讀 §5(被推翻的假設)。** 別再走一次那些死路 —— 特別是別再相信「堆疊封頂 = 修好了」。
3. **做 P0(不收斂偵測器)。** 沒有現場資料,後面全是猜。
4. **做 P2(降低重現成本)。** 否則每次驗證 6.9 小時。
   (P0.5 的 `--extra-ram` 實驗可以和 P2 併著想:如果做得出快速重現,這個實驗就變得很便宜。)
5. 拿到現場資料後,再回頭決定 P1 的 (a)/(b)/(c)。
6. **任何動到 `WireCore` 熱路徑的改動,改完一定重驗黃金 checksum**
   (`--benchmark full_palette.nes --bench-hc 300000 --extra-ram` → 必須是 `0x794A43ABDF169ADA`)。
7. **下次撞到 hang,趁行程還活著先抓 `dotnet-trace`**,再砍掉。活的現場比事後推理值錢太多。
