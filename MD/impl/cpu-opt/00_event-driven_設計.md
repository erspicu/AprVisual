...
| 日期 | commit | phase | 內容 |
|---|---|---|---|
| 2026-05-12 | `<pending>` | branch 建立 + 設計 | `git checkout -b cpu-opt 0a4d758`（S3 末、S4 前）。寫這份設計。base 驗過（`--selftest` 全 PASS、`--trace-cmp --engine ir` 0 mismatch、S1 baseline ~40-43K hc/s / ~15s/幀）。下一步：β.0（`StepOneEventDriven` + `--engine event` + 驗等價）。|
