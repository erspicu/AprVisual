...
Ea=b*8.617e-5
held=[(3.5,0.0),(12.5,0.1),(22.5,0.2),(27.5,0.0),(37.5,-0.5),(47.5,-1.0)]
...
for i in range(n):
    px=lin(xs[i],x0,x1,L,W-Rt); py=lin(ys[i],y0,y1,B,Tp); s+=f'<circle cx="{px:.1f}" cy="{py:.1f}" r="2.3" fill="#7ee0a8"/>'
s+=f'<text x="{W-Rt-8}" y="{Tp+18}" fill="#bfe0f5" font-size="11" text-anchor="end">single-Arrhenius fit: R² = {R2:.5f}</text>'
s+=f'<text x="{W-Rt-8}" y="{Tp+34}" fill="#7f9d88" font-size="10.5" text-anchor="end">Ea = {Ea:.3f} eV (≈ Eg/2) · 52 points, every 1 °C</text>'
s+='</svg>'
open(os.path.join(IMG,"calib_curve.svg"),"w",encoding="utf-8").write(s)

# ---------- Plot 2: residuals vs temperature (lack-of-fit runs) ----------
W,H=660,260; L,Rt,Tp,B=64,20,40,206
rx0,rx1=0,51; ry=max(abs(r) for r in res)*1.15
s=svg_open(W,H,"Residuals of the Arrhenius fit — the runs are lack-of-fit, not noise")
zy=lin(0,-ry,ry,B,Tp)
...
for i in range(n):
    ...
    s+=f'<circle cx="{px:.1f}" cy="{py:.1f}" r="2.3" fill="{col}"/>'
s+=f'<text x="{L+8}" y="{Tp+16}" fill="#8b7f9c" font-size="10">high R² but structured residuals = model mismatch → we use interpolation, not this fit</text>'
s+='</svg>'
...
hx0,hx1=0,51; hy=1.15
s=svg_open(W,H,"Held-out check standards — round-trip error at un-calibrated temps")
# +-0.2 band
b1=lin(0.2,-hy,hy,B,Tp); b2=lin(-0.2,-hy,hy,B,Tp)
...
for (T,e) in held:
    px=lin(T,hx0,hx1,L,W-Rt); py=lin(e,-hy,hy,B,Tp); col="#7ee0a8" if abs(e)<=0.2 else "#ffb454"
...
s+=f'<text x="{L+8}" y="{Tp+16}" fill="#7f9d88" font-size="10">green band = ±0.2 °C · warm end loses sensitivity (slope) → quantization dominates</text>'
s+='</svg>'
open(os.path.join(IMG,"calib_check.svg"),"w",encoding="utf-8").write(s)
print("wrote calib_curve.svg / calib_resid.svg / calib_check.svg  (R2=%.5f Ea=%.3f)"%(R2,Ea))
