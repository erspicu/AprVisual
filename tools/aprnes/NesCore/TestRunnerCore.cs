...
namespace AprNes
{
    ...
    unsafe static class TestRunnerCore
    {
        ...
        public static int Run(string[] args)
        {
            ...
            // ── frame calibration (--calib-json): measure WHEN things happen, for the
            //    switch-level AprVisual.S1 engine (~5 s wall per frame) to budget tightly. ──
            int calibFirstMarkerFrame = -1;      // first frame a terminal marker text is visible
            string calibFirstMarkerKind = null;  // Passed / Failed / $0X / AllComplete / CRC:XXXXXXXX
            int calibLastChangeFrame = 0;        // last frame the screen hash changed
            uint calibPrevHash = 0;
            int calibVerdictFrame = -1;          // frame the verdict was reached
            string calibDetection = null;        // 6000 / screen-stable (null = timeout)
            ...
            handler = (sender, e) =>
            {
                ...
                // --- Calibration scan (--calib-json): every frame, no stability gate ---
                if (calibJsonPath != null && frameCount >= 2)
                {
                    uint chash = 1;
                    for (int ci = 0; ci < 256 * 240; ci += 37)
                        chash = chash * 31 + NesCore.ntsc_rowPalettes[ci];
                    if (chash != calibPrevHash) { calibLastChangeFrame = frameCount; calibPrevHash = chash; }

                    if (calibFirstMarkerFrame < 0)
                    {
                        string kind = null;
                        if (NametableContains("Passed") || NametableContains("PASSED")) kind = "Passed";
                        else if (NametableContains("Failed") || NametableContains("FAILED")) kind = "Failed";
                        else
                        {
                            int oc = DetectOldBlarggScreenCode();
                            if (oc >= 0) kind = "$0" + oc.ToString("X");
                            else if (NametableContains("All tests complete")) kind = "AllComplete";
                            else
                            {
                                string cr = FindNametableCrc();
                                if (cr != null) kind = "CRC:" + cr;
                            }
                        }
                        if (kind != null) { calibFirstMarkerFrame = frameCount; calibFirstMarkerKind = kind; }
                    }
                }
            ...
            if (dumpAcResults)
            {
                ...
                Console.WriteLine(sb.ToString());
                // AC_ref unattended-patch completion block ($07E0-$07FF; magic DE B0 61 at $07F0)
                var sb2 = new StringBuilder("AC_DONE_HEX:");
                for (int addr = 0x07E0; addr < 0x0800; addr++)
                    sb2.Append(NesCore.NES_MEM[addr].ToString("X2"));
            ...
            // --- Calibration record (--calib-json) ---
            if (calibJsonPath != null)
            {
                try
                {
                    string finalCrc = FindNametableCrc();
                    string dir = Path.GetDirectoryName(calibJsonPath);
                    if (!string.IsNullOrEmpty(dir) && !Directory.Exists(dir)) Directory.CreateDirectory(dir);
                    string J(string s) => s == null ? "null" : "\"" + s.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"";
                    var csb = new StringBuilder();
                    csb.Append("{\n");
                    csb.Append("  \"schema\": \"aprnes-calib/1\",\n");
                    csb.Append("  \"rom\": " + J(romName) + ",\n");
                    csb.Append("  \"passed\": " + ((resultCode == 0) ? "true" : "false") + ",\n");
                    csb.Append("  \"resultCode\": " + resultCode + ",\n");
                    csb.Append("  \"detection\": " + J(calibDetection ?? "timeout") + ",\n");
                    csb.Append("  \"framesTotal\": " + frameCount + ",\n");
                    csb.Append("  \"firstMarkerFrame\": " + calibFirstMarkerFrame + ",\n");
                    csb.Append("  \"firstMarkerKind\": " + J(calibFirstMarkerKind) + ",\n");
                    csb.Append("  \"lastChangeFrame\": " + calibLastChangeFrame + ",\n");
                    csb.Append("  \"verdictFrame\": " + calibVerdictFrame + ",\n");
                    csb.Append("  \"autoResets\": " + autoResetCount + ",\n");
                    csb.Append("  \"finalCrc\": " + J(finalCrc) + "\n");
                    csb.Append("}\n");
                    File.WriteAllText(calibJsonPath, csb.ToString());
                    Console.Error.WriteLine("[TestRunner] calib json written: " + calibJsonPath);
                }
